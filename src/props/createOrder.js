...
export function buildOrderActions({ getFacilitatorAccessToken, intent, currency, merchantID, partnerAttributionID } : OrderOptions) : OrderActions {
    const create = (data) => {
        ...
        return createOrderID(order, { getFacilitatorAccessToken, partnerAttributionID, forceRestAPI: false });
    };

    return { create };
}

export function buildPaymentActions({ getFacilitatorAccessToken, intent, currency, merchantID, partnerAttributionID } : OrderOptions) : PaymentActions {
    const create = (data) => {
        ...
        return createPaymentToken(payment, { getFacilitatorAccessToken, partnerAttributionID });
    };

    return { create };
}

export function buildXCreateOrderActions({ getFacilitatorAccessToken, intent, currency, merchantID, partnerAttributionID } : OrderOptions) : XCreateOrderActionsType {
    const order = buildOrderActions({ getFacilitatorAccessToken, intent, currency, merchantID, partnerAttributionID });
    const payment = buildPaymentActions({ getFacilitatorAccessToken, intent, currency, merchantID, partnerAttributionID });
...
export function getCreateOrder({ createOrder, intent, currency, merchantID, partnerAttributionID } : CreateOrderXProps, { getFacilitatorAccessToken, createBillingAgreement, createSubscription } : {| getFacilitatorAccessToken : () => ZalgoPromise<string>, createBillingAgreement? : ?CreateBillingAgreement, createSubscription? : ?CreateSubscription |}) : CreateOrder {
    const data = buildXCreateOrderData();
    const actions = buildXCreateOrderActions({ getFacilitatorAccessToken, intent, currency, merchantID, partnerAttributionID });
